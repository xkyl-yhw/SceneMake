Shader "Custom/ScreenSpaceReflect"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			float alphaCalc(float3 rayDirection,float2 hitPixel, float marchPercent, float hitZ){
				...
				res *=1-max((clamp(abs(screenPCurrent.x),SCREEN_EDGE_MASK,1.0)-SCREEN_EDGE_MASK/(1-SCREEN_EDGE_MASK)),clamp(abs(screenPCurrent.y),SCREEN_EDGE_MASK,1.0)-SCREEN_EDGE_MASK/(1-SCREEN_EDGE_MASK));
				res *= 1-marchPercent;
